...
LRS 的插件系统允许你通过实现标准接口来为文件管理器添加新功能。插件可以做以下事情：
...
> **重要**：插件项目需要引用 LRS 主程序的 DLL（`LRS.dll`），以访问 `IExtension`、`ExtensionContext`、`FileSystemNodeViewModel` 等类型。请将 LRS 主程序编译输出的 `LRS.dll` 复制到插件的引用路径中。
...
| `minAppVersion` | 否 | 最低支持的 LRS 版本，如 `0.0.7` |
...
插件被安装到 `%LocalAppData%\LRS\Plugins\{plugin-id}\`，重启 LRS 后自动加载。
...
- 安装后在 `%LocalAppData%\LRS\Plugins\{id}\` 检查文件是否正确解压
...
- 内置插件：放在 `LRS/Extensions/Extensions/` 目录下
- 外部插件：打包为 `.zip`，通过设置页导入，解压到 `%LocalAppData%\LRS\Plugins\{id}\`
