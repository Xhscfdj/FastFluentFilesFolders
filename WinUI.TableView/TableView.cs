...
public partial class TableView : ListView
{
    ...
    protected override void PrepareContainerForItemOverride(DependencyObject element, object item)
    {
        ...
        // 同步设置 TableView 并登记到 _rows，确保回收容器在首次 Arrange 时 UpdatePosition()
        // 能算出正确的 row.Position（拖拽框选依赖它），且不需要再为每行排队一次 Dispatcher。
        if (element is TableViewRow syncRow)
        {
            syncRow.TableView = this;
            if (!_rows.Contains(syncRow))
            {
                _rows.Add(syncRow);
            }
        }

        // 下面的收尾对本应用（SelectionUnit=Row、只读、无行详情/分组/条件样式）全是空操作，
        // 之前仍每行排队一次 Dispatcher，滚动停止后就会"逐项补齐"（A情况）。仅在真正需要时排队。
        var hasConditionalStyles = (ConditionalCellStyles?.Count > 0)
                                   || Columns.Any(c => c.ConditionalCellStyles?.Count > 0);
        var hasExpandableDetails = RowDetailsVisibilityMode is TableViewRowDetailsVisibilityMode.VisibleWhenExpanded
                                   && (RowDetailsTemplate is not null || RowDetailsTemplateSelector is not null);
        var hasGrouping = _collectionView.GroupDescriptions.Count > 0;
        var needsCellState = SelectionUnit is not TableViewSelectionUnit.Row || CurrentCellSlot.HasValue;

        if (hasConditionalStyles || hasExpandableDetails || hasGrouping || needsCellState)
        {
    ...
    internal bool OnAnyPointerPressed(UIElement pressedElement, PointerRoutedEventArgs e)
    {
        ...
        // 初次按下（尚无移动）直接按锚点选中，不要用零尺寸拖拽矩形去扫描。
        // 矩形扫描依赖 row.Position / 布局，初次按下时可能拿到空或旧位置，
        // 造成"点一个却选中一组相似名称文件"或"选不中"。真实拖拽仍走 OnPointerMoved 的矩形扫描。
        if (LastSelectionUnit is TableViewSelectionUnit.Row && SelectionStartRowIndex is { } startRow)
        {
            MakeSelection(new TableViewCellSlot(startRow, -1), false, ctrlKey);
        }
        else if (SelectionStartCellSlot is { } startCell)
        {
            MakeSelection(startCell, false, ctrlKey);
        }
        else
        {
    ...
    {
        if (_dragStartPoint is null || _lastDragCanvasPoint is null)
        {
            return null;
        }

        // 拖拽起止行：起点优先用按下时记录的锚点（索引可靠），终点用命中测试（不依赖
        // 容器复用后可能陈旧/偏移的 row.Position）。这样切换目录后框选也不会跑偏或失效。
        var startRow = SelectionStartRowIndex ?? GetRowIndexAtCanvasPoint(_dragStartPoint.Value);
        var endRow = GetRowIndexAtCanvasPoint(_lastDragCanvasPoint.Value);
        ...
        var minRow = Math.Min(startRow.Value, endRow.Value);
        var maxRow = Math.Max(startRow.Value, endRow.Value);
    ...
    private void ItemsSourceChanged(DependencyPropertyChangedEventArgs e)
    {
        ...
        // 换数据源时同步取消拖拽选择残留状态。双击进入文件夹时，第二次按下会启动拖拽选择，
        // 但换源发生在拖拽释放之前；晚到的 PointerMoved/释放会再次调用 SelectRowsInDragRect，
        // 拿旧锚点和新数据组合出 0..N 的大范围，把新文件夹"全选"（竖条全亮）。这里清掉拖拽状态即可根治。
        if (IsDragSelecting)
        {
            StopAutoScroll();
            _pointerCaptureElement?.ReleasePointerCaptures();
            ...
            if (_dragRectangle is not null) _dragRectangle.Visibility = Visibility.Collapsed;
    ...
    private int? GetRowIndexAtCanvasPoint(Point canvasPoint)
    {
        ...
        // 命中测试优先：把画布坐标换算到宿主坐标后，直接找可视树里命中的行/单元格。
        // 这比缓存的 row.Position 可靠得多，容器复用后不会偏移。
        try
        {
            var hostPoint = DragRectangleCanvas.TransformToVisual(null).TransformPoint(canvasPoint);
            var hits = VisualTreeHelper.FindElementsInHostCoordinates(hostPoint, this);
            var row = hits.OfType<TableViewCell>().Select(c => c.Row).FirstOrDefault(r => r is not null)
                      ?? hits.OfType<TableViewRow>().FirstOrDefault();
            if (row is not null && row.Index >= 0)
            {
                return row.Index;
            }
        }
